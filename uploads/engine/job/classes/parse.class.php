...
/*
=====================================================
 DataLife Engine - by SoftNews Media Group kaliostro Edition
-----------------------------------------------------
 http://dle-news.ru/
-----------------------------------------------------
		...
		if( $encode ) {
			...
			foreach ( $all_words as $word_line ) {
				...
				if( $word_arr[3] ) {
					...
					$find[] = "#(^|\b|\s|\<br \/\>)" . preg_quote( $word_arr[1], "#" ) . "(\b|!|\?|\.|,|$)#".$register;
					...
					$find[] = "#(" . preg_quote( $word_arr[1], "#" ) . ")#".$register;
